...
public class BoardManager : MonoBehaviour {
	...
	void InitializeList(){
		...
		//Spawn the objects in the middle 6x6 grid
	...
	void BoardSetup(){
		...
		for(int x = -1; x < columns + 1; x++){
			for(int y = -1; y < rows + 1; y++){
				GameObject toInsantiate;

				//If the coordinate is the border
				if(x == -1 || x == columns || y == -1 || y == rows){
					toInsantiate = outerWallTiles[Random.Range(0, outerWallTiles.Length)];
				}
				//If its inside the border
				else{
					toInsantiate = floorTiles[Random.Range(0, floorTiles.Length)];
				}
